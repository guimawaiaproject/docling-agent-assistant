...
class DBManager:
    ...
    async def run_migrations(cls) -> None:
        ...
        def _run_alembic_upgrade() -> None:
            ...
            # apps/api (ou db_manager est dans core/)
            ...
            async with conn.transaction():
                for product in products:
                    try:
                        ...
                        logger.warning("Upsert ignoré pour %s: %s", product.get("designation_raw"), e)
    ...
    async def get_catalogue(
        ...
        async with pool.acquire() as conn:
            ...
            if fournisseur:
                ...
                params.append("%" + _escape_like(fournisseur) + "%")
                idx += 1
    ...
    async def compare_prices(
        ...
        async with pool.acquire() as conn:
            ...
            rows = await conn.fetch(
                "SELECT id, fournisseur, designation_fr, prix_remise_ht, prix_brut_ht, "
                "remise_pct, unite, numero_facture, date_facture, updated_at "
                "FROM produits "
                "WHERE (designation_fr ILIKE $1 ESCAPE E'\\\\' "
                "OR designation_raw ILIKE $1 ESCAPE E'\\\\' "
                "OR similarity(designation_fr, $2) > 0.3)" + user_clause + " "
                "ORDER BY prix_remise_ht ASC LIMIT 20",
                *params,
            )
            return [dict(r) for r in rows]
    ...
    @classmethod
    async def get_user_community_prefs(cls, user_id: int) -> tuple[bool, str | None]:
        """Retourne (community_consent, zone_geo) pour un user."""
        if user_id is None:
            return False, None
        pool = await cls.get_pool()
        async with pool.acquire() as conn:
            row = await conn.fetchrow(
                "SELECT community_consent, zone_geo FROM users WHERE id = $1",
                user_id,
            )
            if not row:
                return False, None
            consent = bool(row.get("community_consent"))
            zone = row.get("zone_geo")
            return consent, str(zone) if zone else None

    @classmethod
    async def update_user_community_prefs(
        cls, user_id: int, community_consent: bool | None = None, zone_geo: str | None = None
    ) -> None:
        """Met à jour les préférences communauté d'un user."""
        pool = await cls.get_pool()
        async with pool.acquire() as conn:
            if community_consent is not None:
                await conn.execute(
                    "UPDATE users SET community_consent = $2 WHERE id = $1",
                    user_id,
                    community_consent,
                )
            if zone_geo is not None:
                await conn.execute(
                    "UPDATE users SET zone_geo = $2 WHERE id = $1",
                    user_id,
                    zone_geo[:10] if zone_geo else None,
                )
