...
describe('useDoclingStore', () => {
  ...
  describe('model selection', () => {
    ...
    test('setModel rejects invalid modelId and falls back to default', () => {
      useDoclingStore.getState().setModel('gemini-2.5-flash')
      useDoclingStore.getState().setModel('invalid-model-id')
      expect(useDoclingStore.getState().selectedModel).toBe('gemini-3-flash-preview')
    })
