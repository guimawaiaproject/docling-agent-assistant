...
import {
  ...
  FileText, Info, Loader2, Package, RefreshCw, Shield, Users, Wifi
} from 'lucide-react'
...
export default function SettingsPage() {
  ...
  useEffect(() => {
    apiClient.get(ENDPOINTS.me).then(({ data }) => {
      setCommunityConsent(data.community_consent ?? false)
      setZoneGeo(data.zone_geo ?? '')
    }).catch(() => {})
  }, [])
  ...
  const saveCommunityPrefs = async () => {
    setCommunitySaving(true)
    try {
      await apiClient.patch(ENDPOINTS.communityPreferences, {
        community_consent: communityConsent,
        zone_geo: zoneGeo || null,
      })
      toast.success('Pr\u00e9f\u00e9rences communaut\u00e9 enregistr\u00e9es')
    } catch {
      toast.error('Impossible de sauvegarder (connexion requise)')
    } finally {
  ...
  return (
    <div className="p-5 min-h-screen bg-[#121212] pb-28">
        ...
        <div className="bg-slate-900 border border-slate-800 rounded-2xl p-4 space-y-4">
          <label className="flex items-center gap-3 cursor-pointer">
            ...
            <span className="text-sm text-slate-300">
              Partager anonymement mes prix pour aider la communaut\u00e9
            </span>
          </label>
          <div>
            <label className="text-[10px] font-bold text-slate-500 uppercase tracking-wider block mb-1">
              Zone g\u00e9ographique (d\u00e9partement FR ou province ES)
            </label>
            <select
              value={zoneGeo}
            ...
            >
              ...
              <optgroup label="France">
                {Array.from({ length: 95 }, (_, i) => `FR-${String(i + 1).padStart(2, '0')}`).map((z) => (
                  <option key={z} value={z}>{z}</option>
                ))}
              </optgroup>
              <optgroup label="Espagne">
                {Array.from({ length: 52 }, (_, i) => `ES-${String(i + 1).padStart(2, '0')}`).map((z) => (
                  <option key={z} value={z}>{z}</option>
                ))}
              </optgroup>
            </select>
          ...
          <motion.button
            whileTap={{ scale: 0.98 }}
            onClick={saveCommunityPrefs}
            disabled={communitySaving}
            className="flex items-center gap-2 px-4 py-2 bg-emerald-600/20 hover:bg-emerald-600/30
              text-emerald-400 font-bold rounded-xl border border-emerald-500/40 disabled:opacity-40"
          >
            {communitySaving ? <Loader2 size={14} className="animate-spin" /> : <CheckCircle2 size={14} />}
            Enregistrer
          </motion.button>
          <p className="text-xs text-slate-500 flex items-start gap-2">
            <Shield size={12} className="mt-0.5 shrink-0" />
            Donn\u00e9es anonymis\u00e9es (hash irr\u00e9versible), k-anonymit\u00e9 sur les stats.
          </p>
        </div>
      </section>
