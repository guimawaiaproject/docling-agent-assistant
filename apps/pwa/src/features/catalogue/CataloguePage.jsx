...
function PriceBar({ products }) {
  ...
  return (
    <div className="bg-slate-800/60 rounded-xl border border-slate-700/50 px-4 py-3 flex items-center gap-6">
      {[
        ...
        <div key={label} className="flex items-center gap-2">
          ...
          <span className={`text-sm font-black ${color}`}>{val.toFixed(2)} €</span>
        </div>
      ))}
    </div>
  )
...
export default function CataloguePage() {
  ...
  }, [famille, fournisseur, search, fetchCatalogue])
  ...
  return (
    <div className="flex flex-col h-screen bg-[#121212]">
      ...
      {/* ── Header sticky ─────────────────────────────────────── */}
      <div className="shrink-0 px-4 pt-4 pb-3 bg-[#121212] border-b border-slate-700/50">
        ...
        <div className="flex gap-2">
          <div className="relative flex-1">
            ...
            <input
              ...
              className="w-full bg-slate-800/60 border border-slate-700/50 rounded-xl py-2.5 pl-9 pr-4
          ...
          <div className="relative w-24 shrink-0">
            ...
            <select
              ...
              className="w-full bg-slate-800/60 border border-slate-700/50 rounded-xl py-2.5 pl-7 pr-2
      ...
      {loading ? (
        <div className="flex-1 p-2">
          <SkeletonCard count={6} variant="catalogue" />
        </div>
      ) : filtered.length === 0 ? (
        <div className="flex-1 flex flex-col items-center justify-center text-slate-500 gap-5 px-6">
          <div className="w-full max-w-xs h-40">
            <EmptyStateIllustration className="w-full h-full min-h-[160px]" />
          </div>
              ...
              <tbody>
                <tr><td colSpan={COLUMNS.length} style={{ height: `${virtualizer.getTotalSize()}px`, position: 'relative', padding: 0 }}>
                  {virtualizer.getVirtualItems().map(vRow => {
                    ...
                    return (
                      <div
                        key={p.id ?? `cat-${(p.designation_fr || '').slice(0, 30)}-${p.fournisseur || ''}-${p.prix_remise_ht ?? 0}`}
                        role="row"
