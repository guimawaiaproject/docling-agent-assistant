...
### Obligatoires (ajoutés en v3)

| Réglage | À quoi ça sert |
|--------|----------------|
| **JWT_SECRET** | Clé secrète pour l'authentification JWT. Générer avec `openssl rand -hex 32`. **L'application ne démarre pas sans.** |
...
| **SENTRY_DSN** | DSN Sentry backend (monitoring erreurs) | (vide = désactivé) |
