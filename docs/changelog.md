...
## [Unreleased] — Refactorisation monorepo

### Changements de structure
- **Arborescence** : `apps/api/` (backend FastAPI + uv), `apps/pwa/` (frontend React)
- **Backend** : `api.py` → `apps/api/main.py`, `backend/*` → `apps/api/core/`, `apps/api/services/`
- **Frontend** : `docling-pwa/*` → `apps/pwa/` avec Feature-Sliced Design
- **Tests** : `apps/api/tests/` (intégration, API, E2E, sécurité)
- **Documentation** : MkDocs centralisé (`mkdocs.yml` racine, `docs/`)
- **Makefile** : cibles `install`, `dev`, `test`, `validate-all`, `docs` adaptées au monorepo
- **CI/CD** : workflows avec `working-directory: apps/api` et `apps/pwa`
- **Pre-commit** : ruff, validate-skills, prettier sur `apps/`

### Dépendances
- **playwright** ajouté aux dev-dependencies pour tests E2E (`apps/api/pyproject.toml`)
